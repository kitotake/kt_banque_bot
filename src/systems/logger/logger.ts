...
export async function logAddMoney(tx: Transaction, targetUsername: string, adminUsername: string): Promise<void> {
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `KT Banque • Tx #${tx.id}` })
    .setTimestamp();

  await sendToChannel(getLogChannelId(), embed);
  await saveLog({ id: generateId(), level: 'INFO', action: 'ADD_MONEY', description: `${adminUsername} +${formatPrex(tx.amount)} → ${targetUsername}`, userId: tx.userId, performedBy: tx.performedBy, metadata: { amount: tx.amount }, timestamp: Date.now() });
}

export async function logRemoveMoney(tx: Transaction, targetUsername: string, adminUsername: string): Promise<void> {
  const embed = new EmbedBuilder()
    .setColor(0xe74c3c)
    .setTitle('💸 Débit admin')
    .addFields(
  ...
  await sendToChannel(getLogChannelId(), embed);
  await saveLog({ id: generateId(), level: 'INFO', action: 'REMOVE_MONEY', description: `${adminUsername} -${formatPrex(tx.amount)} ← ${targetUsername}`, userId: tx.userId, performedBy: tx.performedBy, metadata: { amount: tx.amount }, timestamp: Date.now() });
}
...
export async function logTransfer(txOut: Transaction, fromUsername: string, toUsername: string, adminUsername: string, reason: string): Promise<void> {
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `KT Banque • Tx #${txOut.id}` })
    .setTimestamp();

  await sendToChannel(getLogChannelId(), embed);
  await saveLog({ id: generateId(), level: 'INFO', action: 'TRANSFER', description: `${adminUsername}: ${formatPrex(txOut.amount)} ${fromUsername} → ${toUsername}`, userId: txOut.userId, performedBy: txOut.performedBy, metadata: { amount: txOut.amount, reason }, timestamp: Date.now() });
}
...
export async function logPurchase(purchase: Purchase, username: string, item: ShopItem): Promise<void> {
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `KT Banque • Achat #${purchase.id}` })
    .setTimestamp();

  await sendToChannel(getLogChannelId(), embed);
  await saveLog({ id: generateId(), level: 'INFO', action: 'PURCHASE', description: `${username} acheté "${item.name}" pour ${formatPrex(purchase.price)}`, userId: purchase.userId, performedBy: purchase.userId, metadata: { itemId: item.id, price: purchase.price }, timestamp: Date.now() });
}
...
export async function logRefund(purchase: Purchase, targetUsername: string, adminUsername: string, adminId: string): Promise<void> {
  ...
  await sendToChannel(getLogChannelId(), embed);
  await saveLog({ id: generateId(), level: 'INFO', action: 'REFUND', description: `${adminUsername} remboursé ${formatPrex(purchase.price)} à ${targetUsername}`, userId: purchase.userId, performedBy: adminId, metadata: { purchaseId: purchase.id, price: purchase.price }, timestamp: Date.now() });
}
...
export async function logError(error: Error | string, context?: string): Promise<void> {
  ...
  await saveLog({ id: generateId(), level: 'ERROR', action: 'SYSTEM_ERROR', description: message, metadata: { context }, timestamp: Date.now() });
}
...
export async function logAdminAction(action: string, description: string, adminUsername: string, adminId: string, metadata?: Record<string, unknown>): Promise<void> {
  ...
  await sendToChannel(getLog2ChannelId(), embed);
  await saveLog({ id: generateId(), level: 'INFO', action, description, performedBy: adminId, metadata, timestamp: Date.now() });
}

export async function logFraudAttempt(userId: string, username: string, description: string): Promise<void> {
  const embed = new EmbedBuilder()
    .setColor(0xff4500)
    .setTitle('🚨 Fraude détectée')
    .addFields(
  ...
  await sendToChannel(getLog2ChannelId(), embed);
  await saveLog({ id: generateId(), level: 'CRITICAL', action: 'FRAUD_ATTEMPT', description, userId, timestamp: Date.now() });
}
